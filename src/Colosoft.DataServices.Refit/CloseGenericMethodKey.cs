using System;
using System.Linq;
using System.Reflection;

namespace Colosoft.DataServices.Refit
{
    internal struct CloseGenericMethodKey : IEquatable<CloseGenericMethodKey>
    {
        internal CloseGenericMethodKey(MethodInfo openMethodInfo, Type[] types)
        {
            this.OpenMethodInfo = openMethodInfo;
            this.Types = types;
        }

        public MethodInfo OpenMethodInfo { get; }

        public Type[] Types { get; }

        public bool Equals(CloseGenericMethodKey other) => this.OpenMethodInfo == other.OpenMethodInfo && this.Types.SequenceEqual(other.Types);

        public override bool Equals(object? obj)
        {
            if (obj is CloseGenericMethodKey closeGenericMethodKey)
            {
                return this.Equals(closeGenericMethodKey);
            }

            return false;
        }

        public override int GetHashCode()
        {
            unchecked
            {
                var hash = 17;
                hash = (hash * 23) + this.OpenMethodInfo.GetHashCode();
                foreach (var type in this.Types)
                {
                    hash = (hash * 23) + type.GetHashCode();
                }

                return hash;
            }
        }
    }
}